...
def normalize_inline_math(text: str) -> str:
    ...
    text = pat_single.sub(lambda m: _latex_to_text(m.group(1)), text)
    text = pat_double.sub(lambda m: _latex_to_text(m.group(1)), text)
    return text
...
def make_pages(elements):
    ...
    for kind, text in elements:
        if kind == 'title':
            ...
            content.append('BT /F2 %d Tf 1 1 1 rg %d %d Td (%s) Tj ET' % (TITLE_FONT, MARGIN, 800, esc(text)))
            y = 738
        elif kind == 'h1':
            ensure(2, 20)
